...
export function useRecordPlayer(slugs: string[]) {
  ...
  const stopTimers = useCallback(() => { timers.current.forEach(clearTimeout); timers.current = []; serial.current++; }, []);
  const settle = useCallback(() => {
    stopTimers(); setFlight(null); loadedRef.current = targetRef.current; setLoaded(targetRef.current); setPhase(targetRef.current ? "playing" : "idle");
  }, [stopTimers]);
  const request = useCallback((slug: string | null, manual = false) => {
    stopTimers(); const token = serial.current;
    targetRef.current = slug;
    setFlight(null);
    if (reduced || document.hidden) { loadedRef.current = slug; setLoaded(slug); setPhase(slug ? "playing" : "idle"); return; }
    ...
    const later = (ms: number, fn: () => void) => { timers.current.push(setTimeout(() => { if (serial.current === token) fn(); }, ms)); };
    const incoming = () => {
      ...
      later(420, () => {
        setFlight(null); loadedRef.current = slug; setLoaded(slug); setPhase("lowering");
        later(160, () => { setPhase("playing"); if (manual && soundRef.current) texture(); });
      });
    };
    const outgoing = loadedRef.current;
    if (outgoing === slug && outgoing) { setPhase("playing"); return; }
    if (outgoing) {
      setPhase("lifting");
      later(180, () => {
        const from = destination(), to = origin(outgoing);
        loadedRef.current = null; setLoaded(null); setPhase("returning");
        if (from && to) setFlight({ id: token, slug: outgoing, from, to, returning: true, duration: .24 });
        later(240, () => { setFlight(null); incoming(); });
      });
    } else incoming();
  }, [reduced, stopTimers]);
  ...
  const eject = () => { suppressed.current = activeRef.current; request(null, true); };
  const scratch = () => { if (soundRef.current && loadedRef.current && performance.now() - lastScratch.current > 160) { texture(); lastScratch.current = performance.now(); } };

  useEffect(() => {
    let raf = 0, pending: ReturnType<typeof setTimeout> | undefined, candidate: string | null = null;
    const inspect = () => {
      raf = 0;
      ...
      let index = 0;
      slugs.forEach((slug, i) => { if ((chapters.current.get(slug)?.getBoundingClientRect().top ?? Infinity) <= line) index = i; });
      const previous = activeRef.current ? slugs.indexOf(activeRef.current) : -1;
      if (previous >= 0 && index > previous && (chapters.current.get(slugs[index])?.getBoundingClientRect().top ?? 0) > line - 48) index = previous;
      if (previous >= 0 && index < previous && (chapters.current.get(slugs[previous])?.getBoundingClientRect().top ?? 0) < line + 48) index = previous;
      const next = slugs[index];
      if (next === activeRef.current) { clearTimeout(pending); candidate = null; return; }
      if (candidate === next) return;
      clearTimeout(pending); candidate = next;
      pending = setTimeout(() => {
        activeRef.current = next; setActive(next); candidate = null;
        if (suppressed.current !== next) { suppressed.current = null; request(next); }
      }, 120);
    };
    const scroll = () => { if (!raf) raf = requestAnimationFrame(inspect); };
    const resize = () => { clearTimeout(pending); candidate = null; manualUntil.current = 0; settle(); scroll(); };
    const visibility = () => { clearTimeout(pending); candidate = null; setPageVisible(!document.hidden); settle(); if (!document.hidden) scroll(); };
    const interrupt = () => { manualUntil.current = 0; };
    ...
    const restore = setTimeout(() => { settle(); inspect(); }, 250);
    return () => { clearTimeout(restore); clearTimeout(pending); cancelAnimationFrame(raf); observer.disconnect(); stopTimers(); window.removeEventListener("scroll", scroll); window.removeEventListener("resize", resize); window.removeEventListener("wheel", interrupt); window.removeEventListener("touchstart", interrupt); document.removeEventListener("visibilitychange", visibility); };
  ...
  return { bindScope, bindPlatter, bindSleeve, bindChapter, isOnPlatter, active, loaded, phase, flight, paused, setPaused, sound, setSound, play, eject, scratch, reduced, spinning: !!loaded && phase === "playing" && !paused && visible && pageVisible && !reduced };
}
